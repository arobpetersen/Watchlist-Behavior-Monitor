...
def snapshot_metrics(con, d):
    return con.execute("""
    select count(*) names,
    avg(case when closed_near_high then 1 else 0 end) pct_closed_near_high,
    avg(case when closed_above_vwap then 1 else 0 end) pct_closed_above_vwap,
    avg(case when json_extract(or_1m,'$.broke_orh')::boolean then 1 else 0 end) pct_broke_1m_orh,
    avg(case when json_extract(or_1m,'$.orh_then_orl')::boolean then 1 else 0 end) pct_1m_fakeout,
    avg(case when json_extract(or_5m,'$.broke_orh')::boolean then 1 else 0 end) pct_broke_5m_orh,
    avg(case when json_extract(or_5m,'$.orh_then_orl')::boolean then 1 else 0 end) pct_5m_fakeout,
    median(close_location) median_close_location
...
def snapshot_table(con, d):
    ...
    select c.ticker,c.rating,c.setup,c.focus,b.primary_label,b.secondary_labels,
           f.close_location,f.closed_above_vwap,f.or_1m,f.or_5m,f.or_15m
    ...
    return df[['ticker','rating','setup','focus','primary_label','secondary_labels','close_location','closed_above_vwap','1m OR result','5m OR result','15m OR result']]
