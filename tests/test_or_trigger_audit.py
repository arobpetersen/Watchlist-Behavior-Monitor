...
def test_audit_fields_and_break_rows_use_strict_comparisons():
    ...
    assert row['1m Low Swept Before Trigger'] == ''
    assert row['1m ORH Reference Low'] == '1.00'
    assert row['1m ORH Reference Basis'] == 'LOD at 1m Trigger'
    assert row['1m Post-Trigger Stop Breach'] == 'Yes'
    ...
    assert row['Displayed PDH'] == 'Gap'
    assert row['PDH Result'] == 'Gap'
    assert row['5m ORH Attempted'] == 'Yes'
    assert row['5m Low Swept Before Trigger'] == ''
    assert row['5m ORH Reference Low'] == '0.99'
    assert row['5m ORH Reference Basis'] == 'LOD at 5m Trigger'
    assert row['5m Post-Trigger Stop Breach'] == 'Yes'
